...
def extract_colors_from_css(css_path):
    ...
    try:
        with open(css_path, 'r', encoding='utf-8') as file:
            ...
            # Use regex to find color codes in the CSS (e.g., #ffffff, rgb(255,255,255))
...
def extract_colors(url):
    website_name = urlparse(url).netloc  # Use the domain as the folder name
    images_dir = os.path.join('scraped', website_name, 'images')
    css_dir = os.path.join('scraped', website_name, 'css')
    output_dir = os.path.join('scraped', website_name)  # Folder where the text file will be saved
    ...
    if os.path.exists(images_dir):
        for image_file in os.listdir(images_dir):
            if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):  # Only process supported formats
    ...
    distinct_css_colors = set()  # Use set to avoid duplicates
    ...
    # Convert the set to sorted lists
    ...
    # Prepare lists for writing to text file
